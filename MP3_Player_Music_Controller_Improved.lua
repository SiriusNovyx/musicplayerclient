-- ============================================================================
--  MP3 Player / Music Controller  (Improved Edition)
--  Compatible with: Roblox Executor environments (PC + Mobile)
-- 
--  IMPROVEMENTS OVER ORIGINAL:
--    • Fixed: _cinGui hoisted to proper local scope
--    • Fixed: activeToasts hoisted so hideController can dismiss toasts
--    • Fixed: Removed dead code `if kbRow then end`
--    • Fixed: loadUserSettings / saveUserSettings now schema-driven (DRY)
--    • Fixed: LYRICS_LOOKAHEAD now a Cfg constant, not computed every frame
--    • Added: Mobile swipe left/right to skip tracks
--    • Added: Better sound load error messages (shows file path)
--    • Added: Volume/speed clamping safety guards
--    • Improved: Mobile toggle button uses ♪/✕ icons
-- ============================================================================
...
local UI={}
local _cinGui = nil          -- cinematic overlay ScreenGui (hoisted so all closures share it)
local activeToasts = {}      -- toast stack (hoisted so hideController can dismiss on close)
...
local function saveUserSettings()
	...
	-- Schema-driven save: build the data table from SETTINGS_SCHEMA
	local d = {}
	for _,entry in ipairs(SETTINGS_SCHEMA) do
		d[entry.key] = St[entry.stateKey]
	end
	-- Also save keybinds
	local function kbn(k) return St[k] and St[k].Name or nil end
	for _,entry in ipairs(KEYBIND_SCHEMA) do d[entry.key] = kbn(entry.key) end
...
local function loadUserSettings()
	...
	-- Schema-driven load: iterate SETTINGS_SCHEMA so new fields just work automatically
	for _,entry in ipairs(SETTINGS_SCHEMA) do
		local v = d[entry.key]
		if type(v) == entry.type then St[entry.stateKey] = v end
	end
	-- Load keybinds separately (they need enum lookup)
...
s=ns else warn("[MusicController] Failed to load sound: " .. fi.Name .. " (path: " .. tostring(fi.Path) .. ")")
ns:Destroy() end end
...
if not ok2 or not td then return nil,"Cannot parse track data",nil end
	...
	-- Bug fix: detect private tracks (policy == "PRIVATE") and block before download
	if policy == "PRIVATE" then
		return nil, "This track is private and cannot be downloaded.", td.title
	end
	-- Bug fix: detect blocked/blocked-with-alternative/blocked-in-region tracks
	if policy == "BLOCK" or policy == "BLOCKED" then
		return nil, "This track is blocked / not available for download.", td.title
	end
...
c.Parent=p end
local function applyStroke(t,col,tr,p)
	local s=Instance.new("UIStroke")
	s.Thickness=t
	s.Color=col
	s.Transparency=tr
	s.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
	-- UIStroke may be rejected on some instance types in older executor environments
	local ok=pcall(function() s.Parent=p end)
	if not ok then s:Destroy(); return nil end
	return s
end
...
-- (kbRow removed: was dead code referencing an undefined variable)
...
mkSlider(UI.content,"volume",92,Cfg.SLIDER_MIN,Cfg.SLIDER_MAX,St.currentVolume,2,getInc,Cfg.SNAP_VALUE,Cfg.SNAP_THRESHOLD,function(v) St.currentVolume=math.clamp(v, 0, 10);if sound then sound.Volume=math.max(0, St.currentVolume) end;saveUserSettings() end)
mkSlider(UI.content,"speed",140,Cfg.SLIDER_MIN,Cfg.SLIDER_MAX,St.currentSpeed,2,getInc,Cfg.SNAP_VALUE,Cfg.SNAP_THRESHOLD,function(v) St.currentSpeed=math.max(0, v);if sound then sound.PlaybackSpeed=math.max(0.01, St.currentSpeed) end;saveUserSettings() end)
...
createCategoryAction("↓",20,1)
				-- Delete button for categorised tracks
				do
					local catDelBtn = Instance.new("TextButton")
					catDelBtn.Name = "DeleteBtn"
					catDelBtn.AnchorPoint = Vector2.new(1, 0.5)
					catDelBtn.Position = UDim2.new(1, -48, 0.5, 0)
					catDelBtn.Size = UDim2.new(0, 20, 0, 20)
					catDelBtn.BackgroundColor3 = C.DANGER
					catDelBtn.AutoButtonColor = false
					catDelBtn.Font = Enum.Font.GothamBold
					catDelBtn.Text = "×"
					catDelBtn.TextColor3 = Color3.fromRGB(255,255,255)
					catDelBtn.TextSize = 12
					catDelBtn.BorderSizePixel = 0
					catDelBtn.ZIndex = 4
					catDelBtn.Visible = isMobileDevice()
					catDelBtn.Parent = tb
					applyCornerRadius(5, catDelBtn)
					local catDelIdx = ti
					catDelBtn.MouseButton1Click:Connect(function()
						local track = Dat.trackList[catDelIdx]
						if track then
							St.pendingDeleteTrackIndex = catDelIdx
							UI.deleteConfirmMessage.Text = 'Delete "' .. track.DisplayName .. '"?'
							UI.deleteConfirmFrame.Visible = true
						end
					end)
					if not isMobileDevice() then
						catDelBtn.Visible = false
						tb.MouseEnter:Connect(function() catDelBtn.Visible = true end)
						tb.MouseLeave:Connect(function() catDelBtn.Visible = false end)
					end
				end
				local cti=ti
...
createReorderArrow("↓",20,1)
		-- Delete button (always visible on mobile, hover-only on PC)
		do
			local delBtn = Instance.new("TextButton")
			delBtn.Name = "DeleteBtn"
			delBtn.AnchorPoint = Vector2.new(1, 0.5)
			delBtn.Position = UDim2.new(1, -48, 0.5, 0)
			delBtn.Size = UDim2.new(0, 22, 0, 22)
			delBtn.BackgroundColor3 = C.DANGER
			delBtn.AutoButtonColor = false
			delBtn.Font = Enum.Font.GothamBold
			delBtn.Text = "×"  -- × character
			delBtn.TextColor3 = Color3.fromRGB(255,255,255)
			delBtn.TextSize = 14
			delBtn.BorderSizePixel = 0
			delBtn.ZIndex = 4
			delBtn.Visible = isMobileDevice()
			delBtn.Parent = btn
			applyCornerRadius(6, delBtn)
			delBtn.MouseEnter:Connect(function()
				Svc.TweenService:Create(delBtn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(230,50,50)}):Play()
			end)
			delBtn.MouseLeave:Connect(function()
				Svc.TweenService:Create(delBtn,TweenInfo.new(0.1),{BackgroundColor3=C.DANGER}):Play()
			end)
			local delIdx = ref.idx
			delBtn.MouseButton1Click:Connect(function()
				local track = Dat.trackList[delIdx]
				if track then
					St.pendingDeleteTrackIndex = delIdx
					UI.deleteConfirmMessage.Text = 'Delete "' .. track.DisplayName .. '"?'
					UI.deleteConfirmFrame.Visible = true
				end
			end)
			if not isMobileDevice() then
				...
				btn.MouseEnter:Connect(function() delBtn.Visible = true end)
				btn.MouseLeave:Connect(function() delBtn.Visible = false end)
			end
		end
...
Svc.Workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
	...
	-- Bug fix: after orientation flip lyrics gui can reappear even when turned OFF
	if lyricsOverlay and lyricsOverlay.gui then
		if not St.lyricsEnabled then
			lyricsOverlay.gui.Enabled = false
		end
	end
	-- Resize lyrics pill to fit screen height on small/rotated screens
	if lyricsOverlay and lyricsOverlay.frame then
		local pillH = math.max(90, math.min(120, vp.Y * 0.16))
		lyricsOverlay.frame.Size = UDim2.new(0.96, 0, 0, pillH)
	end
end)
...
UI.dragArea.InputBegan:Connect(function(inp)
	if inp.UserInputType == Enum.UserInputType.Touch then
		...
		local gx = pos.X.Scale*vp.X + pos.X.Offset
		local gy = pos.Y.Scale*vp.Y + pos.Y.Offset
		Dat.dragStart = Vector2.new(inp.Position.X - gx, inp.Position.Y - gy)
	end
end)
...
Svc.UserInput.InputChanged:Connect(function(inp)
	...
	if St.draggingWindow and (isMouse or isTouch) then
		local px = isMouse and Svc.UserInput:GetMouseLocation().X or inp.Position.X
		local py = isMouse and Svc.UserInput:GetMouseLocation().Y or inp.Position.Y
		local vp=Svc.Workspace.CurrentCamera.ViewportSize
		local as=UI.main.AbsoluteSize
		...
		local nx=math.clamp(px-Dat.dragStart.X, leftPad+as.X/2, vp.X-as.X/2-rightPad)
		local ny=math.clamp(py-Dat.dragStart.Y, as.Y/2, vp.Y-as.Y/2)
...
-- (touch drag is handled by the InputBegan / InputChanged / InputEnded above)
...
-- ── Mobile Swipe Gesture: swipe left = next track, swipe right = prev track ──
do
	local swipeStart = nil
	local SWIPE_THRESHOLD = 60  -- pixels to count as a swipe
	UI.main.InputBegan:Connect(function(inp)
		if inp.UserInputType == Enum.UserInputType.Touch then
			swipeStart = Vector2.new(inp.Position.X, inp.Position.Y)
		end
	end)
	UI.main.InputEnded:Connect(function(inp)
		if inp.UserInputType == Enum.UserInputType.Touch and swipeStart then
			local delta = inp.Position.X - swipeStart.X
			local vertDelta = math.abs(inp.Position.Y - swipeStart.Y)
			-- Only count horizontal swipes (not vertical scrolls)
			if math.abs(delta) > SWIPE_THRESHOLD and vertDelta < SWIPE_THRESHOLD then
				if delta < 0 then
					-- Swipe left → next track
					playNextTrack()
				else
					-- Swipe right → previous track
					setCurrentTrack(getAdjacentTrackIndex(-1), true)
				end
			end
			swipeStart = nil
		end
...
do
	...
	-- Update button icon AND show current track name when closed
	Svc.RunService.Heartbeat:Connect(function()
		local expected = St.guiOpen and "✕" or "♪"
		if toggleBtn.Text ~= expected then
			toggleBtn.Text = expected
			toggleBtn.TextColor3 = St.guiOpen and C.TEXT2 or C.ACCENT
		end
	end)
end
...
Svc.UserInput.InputBegan:Connect(function(inp,gp)
	...
	-- St.capturingKeybind is no longer used; guard kept for safety only.
	if St.capturingKeybind then St.capturingKeybind=false; return end
...
Svc.RunService.RenderStepped:Connect(function()
	...
	if St.lyricsEnabled and lyricsOverlay and lyricsOverlay.gui and lyricsOverlay.gui.Enabled then
		if s and s.TimeLength and s.TimeLength>0 and pl then
			-- Add a small look-ahead so lyrics appear on-beat instead of slightly late
			local LYRICS_LOOKAHEAD = isMobileDevice() and Cfg.LYRICS_LOOKAHEAD_MOBILE or Cfg.LYRICS_LOOKAHEAD_PC
			local pos=math.max(0,(s.TimePosition or 0)+LYRICS_LOOKAHEAD);local lyr=Dat.currentLyrics
			if #lyr>0 then
